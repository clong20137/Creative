...
export const licensesAPI = {
  ...
  reactivateClientLicense: (clientId: string) => unwrap(api.put(`/subscriptions/client/${clientId}/license/reactivate`))
}
...
export const pluginsAPI = {
  ...
  getBlogPosts: () => unwrap<{ plugin: any; posts: any[] }>(api.get('/plugins/blog/posts')),
  getBlogPost: (slug: string) => unwrap<{ plugin: any; post: any }>(api.get(`/plugins/blog/posts/${slug}`)),
...
export const adminAPI = {
  ...
  getAuditLogs: (params?: { action?: string; targetType?: string; q?: string; limit?: number }) => unwrap<any[]>(api.get('/admin/audit-logs', { params })),
  ...
  bulkUpdateMedia: (data: any) => unwrap<any[]>(api.put('/admin/media/bulk', data)),
  bulkDeleteMedia: (ids: Array<string | number>) => unwrap(api.delete('/admin/media/bulk', { data: { ids } })),
  ...
  getBlogPosts: () => unwrap<any[]>(api.get('/admin/plugins/blog/posts')),
  createBlogPost: (data: any) => unwrap(api.post('/admin/plugins/blog/posts', data)),
  updateBlogPost: (id: string, data: any) => unwrap(api.put(`/admin/plugins/blog/posts/${id}`, data)),
  deleteBlogPost: (id: string) => unwrap(api.delete(`/admin/plugins/blog/posts/${id}`)),
  ...
  getPageRevisions: (id: string) => unwrap<any[]>(api.get(`${isBuilderSession() ? '/builder/pages' : '/admin/pages'}/${id}/revisions`)),
  restorePageRevision: (id: string, revisionId: string) => unwrap(api.post(`${isBuilderSession() ? '/builder/pages' : '/admin/pages'}/${id}/revisions/${revisionId}/restore`)),
  ...
  getBuiltInPageRevisions: (pageKey: string) => unwrap<any[]>(api.get(`${isBuilderSession() ? '/builder/site-settings' : '/admin/site-settings'}/page-revisions/${pageKey}`)),
  restoreBuiltInPageRevision: (pageKey: string, revisionId: string) => unwrap(api.post(`${isBuilderSession() ? '/builder/site-settings' : '/admin/site-settings'}/page-revisions/${pageKey}/${revisionId}/restore`)),
  ...
  exportStoredBackup: (id: string) => unwrap<any>(api.get(`/admin/backups/${id}/export`)),
